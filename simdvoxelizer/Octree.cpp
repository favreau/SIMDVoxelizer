...
Octree::Octree(const std::vector<float> &events, float voxelSize,
    ...
    : _volumeDim(glm::uvec3(0u, 0u, 0u)), _volumeSize(0u),
      _offsetPerLevel(nullptr), _flatIndexes(nullptr), _flatData(nullptr) {
  ...
  for (uint32_t i = 0; i < events.size(); i += 5) {
    ...
    const uint64_t ypos = std::floor((events[i + 1] - minAABB.y) / voxelSize);
    const uint64_t zpos = std::floor((events[i + 2] - minAABB.z) / voxelSize);
    ...
    if (it == octree[0].end()) {
      ...
      for (uint32_t level = 0; level < octreeDepth; ++level) {
        ...
        if (octree[level].find(index) == octree[level].end()) {
          octree[level].insert(
              OctreeLevelMap::value_type(index, OctreeNode(center, size)));
          newNode = true;
        }
  ...
  for (uint32_t i = 0; i < octree.size(); ++i)
    std::cout << "Number of leaves [" << i << "]: " << octree[i].size()
              << std::endl;
  ...
  _volumeSize =
      (uint64_t)_volumeDim.x * (uint64_t)_volumeDim.y * (uint64_t)_volumeDim.z;
}
...
uint32_t Octree::getOctreeSize() const { return _octreeSize; }

uint32_t *Octree::getFlatIndexes() const { return _flatIndexes; }

float *Octree::getFlatData() const { return _flatData; }

const glm::uvec3 &Octree::getVolumeDim() const { return _volumeDim; }

uint64_t Octree::getVolumeSize() const { return _volumeSize; }
